...
class DecryptionTests extends APISpec {
    ...
    def "Decryption"() {
        ...
        metadata.put(Constants.EncryptionConstants.ENCRYPTION_DATA_KEY, new ObjectMapper().writeValueAsString(flow.getEncryptionData()))
        ...
        def decryptedData = collectBytesInBuffer(
            blobEncryptionPolicy.decryptBlob(metadata, flow, new EncryptedBlobRange(blobRange), true)).block()
