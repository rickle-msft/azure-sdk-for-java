...
public final class DirectoryURL extends StorageURL {
     ...
     *         Identifies the portion of the list to be returned with the next list operation. This value
     *         is returned in the response of a previous list operation as the
     *         DirectoryListHandlesResponse.body().nextMarker(). Set to null to list
     *         the first segment.
     *
     * @return Emits the successful response.
     */
    public Single<DirectoryListHandlesResponse> listHandles(String marker) {
        return listHandles(marker, null, true, Context.NONE);
    }
     ...
     * @return Emits the successful response.
     */
    public Single<DirectoryListHandlesResponse> listHandles(String marker, Integer maxresults, boolean recursive, Context context) {
        context = context == null ? Context.NONE : context;

        return addErrorWrappingToSingle(this.storageClient.generatedDirectorys()
                .listHandlesWithRestResponseAsync(context, marker, maxresults, null, null, recursive == false ? null : true));
    }
     ...
     *
     * @param marker
     *         Marker from the previous forceCloseHandles() request, if there were too many file handles to be closed in one call.
     *         If all file handles were closed, marker will be null.
     *
     * @return Emits the successful response.
     */
    public Single<DirectoryForceCloseHandlesResponse> forceCloseHandles(String marker) {
        return forceCloseHandles(ALL_HANDLES, marker, true, Context.NONE);
    }

    /**
     * Closes specified file handle open for given directory.
     *
     * @param handleId
     *         Identifies the files handle to close.  Pass "*" to close all handles.
     * @param marker
     *         Marker from the previous forceCloseHandles() request, if there were too many file handles to be closed in one call.
     *         If all file handles were closed, marker will be null.
     * @param recursive
     ...
     * @return Emits the successful response.
     */
    public Single<DirectoryForceCloseHandlesResponse> forceCloseHandles(String handleId, String marker, boolean recursive, Context context) {
        context = context == null ? Context.NONE : context;

        return addErrorWrappingToSingle(this.storageClient.generatedDirectorys()
                .forceCloseHandlesWithRestResponseAsync(context, handleId, null, marker, null, recursive == false ? null : true));
    }
}
