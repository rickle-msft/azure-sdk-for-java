...
public final class FileURL extends StorageURL {
     ...
     *         FileListHandlesResponse.body().nextMarker(). Set to null to list
     *
     * @return Emits the successful response.
     */
    public Single<FileListHandlesResponse> listHandles(String marker) {
        return listHandles(marker, null, Context.NONE);
    }
     ...
     *         FileListHandlesResponse.body().nextMarker(). Set to null to list
     * @param maxresults
     *         Specifies the maximum number of files to return, including all prefix elements.
     * @param context
     *         {@code Context} offers a means of passing arbitrary data (key/value pairs) to an
     *         {@link com.microsoft.rest.v2.http.HttpPipeline}'s policy objects. Most applications do not need to pass
     *         arbitrary data to the pipeline and can pass {@code Context.NONE} or {@code null}. Each context object is
     ...
     * @return Emits the successful response.
     */
    public Single<FileListHandlesResponse> listHandles(String marker, Integer maxresults, Context context) {
        context = context == null ? Context.NONE : context;

        return addErrorWrappingToSingle(this.storageClient.generatedFiles()
                .listHandlesWithRestResponseAsync(context, marker, maxresults, null, null));
    }
     ...
     * @return Emits the successful response.
     */
    public Single<FileForceCloseHandlesResponse> forceCloseHandles(String marker) {
        return forceCloseHandles(ALL_HANDLES, marker, Context.NONE);
    }

    /**
     * Closes specified handle for given file.
     *
     * @param handleId
     ...
     * @return Emits the successful response.
     */
    public Single<FileForceCloseHandlesResponse> forceCloseHandles(String handleId, String marker, Context context) {
        context = context == null ? Context.NONE : context;

        return addErrorWrappingToSingle(this.storageClient.generatedFiles()
                .forceCloseHandlesWithRestResponseAsync(context, handleId, null, marker, null));
    }
}
