...
public class EncryptedFlowableForTest extends Flowable<ByteBuffer> {
    ...
    */
    ...
    public EncryptedFlowableForTest(int testCase, IKey key) throws InvalidKeyException {
        ...
        this.plainText = APISpec.getRandomData(DOWNLOAD_SIZE - 2); // This will yield two bytes of padding... for fun.

        EncryptedBlob encryptedBlob = new BlobEncryptionPolicy(key, null, false)
            .encryptBlob(Flowable.just(this.plainText)).blockingGet();
        this.cipherText = FlowableUtil.collectBytesInBuffer(encryptedBlob.getCipherTextFlowable()).blockingGet();
